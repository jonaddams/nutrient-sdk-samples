...
import type { Annotation, NutrientInstance } from "@/lib/types/nutrient";
import {
	COMPARISON_CONFIG,
	HIGHLIGHT_COLORS,
	SELECTORS,
...
export default function TextComparisonPage() {
	...
	async function compareDocuments() {
		...
		const changedScrollElement =
			changedInstance.contentDocument.querySelector(SELECTORS.SCROLL_CONTAINER);
		...
		const originalScrollElement =
			originalInstance.contentDocument.querySelector(SELECTORS.SCROLL_CONTAINER);
		...
		if (operation.insert && operation.del) {
			return (
				<div className="text-xs">
					<span className="bg-[#FFC9CB] dark:bg-[#8B3A3A] px-1 rounded text-black dark:text-white">-{deleteCount}</span>
					{" | "}
					<span className="bg-[#C0D8EF] dark:bg-[#3A5A7A] px-1 rounded text-black dark:text-white">+{insertCount}</span>
				</div>
			);
		}
		if (operation.insert) {
	...
	async function highlightSelectedChange(operation: ChangeOperation) {
		if (!originalInstanceRef.current || !changedInstanceRef.current || !window.NutrientViewer) return;
			...
			<header className="border-b border-[var(--warm-gray-400)] bg-white dark:bg-[#1a1414]">
				<div className="max-w-full mx-auto px-6 py-4">
					...
					<p className="text-lg opacity-80">
						Side-by-side PDF comparison with synchronized viewing and interactive
						change tracking
					</p>
				</div>
			</header>
