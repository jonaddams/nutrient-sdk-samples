...
export default function OcrExtractionPage() {
  ...
  const handleDownload = () => {
    ...
    a.href = url;
    a.download = `${selected.filename.replace(/\.[^.]+$/, "")}-ocr.json`;
    a.click();
  ...
  return (
    <div className="min-h-screen" style={{ background: "var(--bg)" }}>
      <PythonSampleHeader
        title="Extracting data from images using OCR"
        description="Use Adaptive OCR to extract text from images for high-throughput workflows. Best on printed text where speed matters; for hand-printed or mixed-content forms, use ICR instead."
      />
          ...
          <div className="flex h-full">
            ...
            <div className="w-80 border-r border-[var(--line)] bg-[var(--bg-elev)] flex flex-col flex-shrink-0">
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4">
                ...
                >
                  {processing ? "Extracting..." : "Extract data with OCR"}
                </button>

                <div className="p-3 rounded-md border border-[var(--line)] bg-[var(--surface)] text-xs text-[var(--ink-3)] leading-relaxed">
                  <p className="font-medium text-[var(--ink-2)] mb-1">
                    OCR vs ICR
                  </p>
                  <p>
                    OCR (Adaptive OCR) is optimized for high-throughput
                    extraction of printed text. ICR handles a broader range
                    including hand-printed forms but is heavier. Reach for OCR
                    first when the input is purely printed; switch to ICR when
                    hand-printed content is present.
                  </p>
                </div>
